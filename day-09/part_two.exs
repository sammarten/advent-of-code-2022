defmodule RopeBridge do
  ...
  def run_commands(commands) do
    context = %{h: {0, 0}, knots: List.duplicate({0, 0}, 9), visited: MapSet.new([{0, 0}])}
  ...
  def take_step(direction, context) do    
    %{h: h, knots: knots, visited: visited} = context

    updated_h = update_h(direction, h)
    updated_knots = update_knots(updated_h, knots)
    updated_visited = update_visited(visited, List.last(updated_knots))
  ...
  def update_knots(h, knots) do
    knots
    |> Enum.reduce(%{prev: h, updated_knots: []}, &update_knot/2)
    |> Map.get(:updated_knots)
  end

  def update_knot(knot, acc) do
    h = acc.prev
    t = knot
    ...
    %{prev: updated_knot, updated_knots: acc.updated_knots ++ [updated_knot]}
  end
